...
float Vertex::laplacian() const {
  // TODO (Animation) Task 4
	...
	VertexCIter i = h->vertex();
	float lap = 0.;

	do {
		VertexCIter j = h->next()->vertex();
		VertexCIter a = h->next()->next()->vertex();
		VertexCIter b = h->twin()->next()->next()->vertex();

		Vector3D ai = i->position - a->position;
		Vector3D aj = j->position - a->position;
		Vector3D bi = i->position - b->position;
		Vector3D bj = j->position - b->position;

		float angle_a = acos(dot(ai, aj) / (ai.norm() * aj.norm()));
		float angle_b = acos(dot(bi, bj) / (bi.norm() * bj.norm()));

		float cot_a = cos(angle_a) / sin(angle_a);
		float cot_b = cos(angle_b) / sin(angle_b);

		lap += (cot_a + cot_b) * (j->offset - i->offset);
