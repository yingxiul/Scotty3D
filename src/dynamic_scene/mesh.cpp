  ...
  // TODO (Animation) Task 3a, Task 3b
	...
	for (VertexIter v = mesh.verticesBegin(); v != mesh.verticesEnd(); v++) {
		...
		for (size_t i = 0; i < skeleton->joints.size(); i++) {
			...
			// vertex position relatively to joint before transformation
			Vector3D v_pos = j->getBindTransformation().inv() * v->bindPosition;
			// vertex position after transformation in world coordinate
			Vector3D blendPos = j->getTransformation() * j->getRotation() * v_pos;
			...
			if (det <= 0) {
				...
				dist = cross(v, v - u).norm() / u.norm();
			}

			// check if add it to dist_sum
			if (!useCapsuleRadius || (useCapsuleRadius && dist <= j->capsuleRadius)) {
				dist_sum += (1.0 / dist);
				LBSInfo info;
		...
		// compute the position using the given two expression
		if (infos.size() == 0) {
			...
			for (int k = 0; k < infos.size(); k++) {
				v->position += (1. / infos[k].distance / dist_sum) * infos[k].blendPos;
			}
		}
	}
}
...
void Mesh::forward_euler(float timestep, float damping_factor) {
  // TODO (Animation) Task 4
	vector<float> new_vel;

	for (VertexIter v = mesh.verticesBegin(); v != mesh.verticesEnd(); v++) {
		float delta = v->laplacian() - damping_factor * v->velocity;
		float vel = v->velocity + timestep * delta;
		new_vel.push_back(vel);
	}
