...
Ray Camera::generate_ray(double x, double y) const {
  ...
  //compute x,y coordinate for screen top-right corner
	double trX = tan(radians(hFov)*0.5);
	double trY = tan(radians(vFov)*0.5);
	//compute x,y coordinates for screen bottom-left corner
	double brX = -trX;
	double brY = -trY;
	//interpolate to get the intersection x,y coordinate
	double camX = brX + x*(trX - brX);
	double camY = brY + y*(trY - brY);
	//convert to world
	Vector3D cam = Vector3D(camX, camY, -1);
	//cout << "cam: " << cam << endl;
	Vector3D dir = c2w*cam;
	//dir.normalize();
	//Vector3D h = dir.unit();
	//return Ray(pos, h);
