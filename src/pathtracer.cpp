...
// log ray hit
  ...
  if (!isect.bsdf->is_delta()) {
    ...
    for (SceneLight* light : scene->lights) {
      ...
      for (int i = 0; i < num_light_samples; i++) {
		...
		Ray back_ray = Ray(hit_p + EPS_D*dir_to_light, dir_to_light, dist_to_light-EPS_D, 0);
		Intersection back_isect;
		back_isect.t = dist_to_light - EPS_D;
		bool back_hit = bvh->intersect(back_ray, &back_isect);
  ...
  wi = o2w*wi;
  wi.unit();
  //cout << "f.illm(): " << f.illum()<<endl;
  Ray ray_i(hit_p + EPS_D*wi, wi, INF_D, r.depth + 1);


  // (2) potentially terminate path (using Russian roulette)
  float terminateProbability = clamp(1.0 - f.illum(), 0., 1.);
  //cout << "terminateProbability: " << terminateProbability << endl;
  ...
  //cout << "L_out prev: " << L_out << endl;
  if (randomFloat > terminateProbability && r.depth < max_ray_depth) {

	  L_out += (f * trace_ray(ray_i) * fabs((float)dot(wi,isect.n)) * (1 /(pdf*(1. - terminateProbability))));
  }  
  //cout << "L_out after: " << L_out << endl;
...
Spectrum PathTracer::raytrace_pixel(size_t x, size_t y) {
  ...
  // The sample rate is given by the number of camera rays per pixel.
	...
	if (num_samples == 1) {
		...
		//cout << "x: " << x << endl;
		//cout << "y: " << y << endl;
		ctrX = (x + 0.5) / (double)sampleBuffer.w;
		ctrY = (y + 0.5) / (double)sampleBuffer.h;
		//cout << "ctrx: " << ctrX << endl;
		//cout << "ctry: " << ctrY << endl;
		Ray r = camera->generate_ray(ctrX, ctrY);
		//r.depth = max_ray_depth;
		return trace_ray(r);
	}
	else {
		for (int i = 0; i < num_samples; i++) {
			v = gridSampler->get_sample();
			ctrX = (x + v.x) / (double)sampleBuffer.w;
			ctrY = (y + v.y) / (double)sampleBuffer.h;
			Ray r = camera->generate_ray(ctrX, ctrY);
			//r.depth = max_ray_depth;
			s += trace_ray(r);
		}
		return s * (1 / (double)num_samples);
	}
}
