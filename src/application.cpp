...
void Application::toggle_OCamera_action() {
	//if (action != Action::OCamera) {
		//action = Action::OCamera;

		char videoPrefix[100] = "C:\\Users\\Jessy\\Desktop\\CMU\\2018Spring\\sth\\O camera\\OCamera_scotty\\build\\src\\";
		//scene->triangulateSelection();
		...
		for (int i = 0; i < 16; i++) {
			...
			char buffer[50];
			sprintf(buffer, "camera_%d_%d.png", i , totalFrame);
			//cout << "aaaaaa" << endl;
			totalFrame += 1;
			pathtracer->save_image(videoPrefix + string(buffer));
		}
		//camera.rotate_by(0, 0.125 * PI);
		pathtracer->stop();
		pathtracer->clear();
	...
	//else {
	//	action = Action::Navigate;
	//	// close 2nd window and clear everything
	//}
}
